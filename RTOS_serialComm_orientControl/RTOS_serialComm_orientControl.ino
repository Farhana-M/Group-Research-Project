...
// Orientation Controller by Farhana Moosa
...
TaskHandle_t Handle_pingMotorTask;
...
static void motorControl(void *pvParameters)
{
  while(1)
  {
    ...
    // Serial.print(theta_x_measured);
    // Serial.print(" || Theta y:");
    // Serial.print(theta_y_measured);
    // myDelayMs(1000);
    ...
    for (int i = 0; i < 3; i++) {
      for (int k = 0; k < 2; k++) {
        ...
        // Serial.println("");
        // Serial.print("Current Delta L:");
        // Serial.print(currentDeltaL[i]);
      }
    }
...
static void pingMotor(void *pvParameters)
// ping motor AND set operating mode
{
  myDelayMs(1000); // do not remove delay as it needs time to ping the motor
  for(int x=0; x<1; ++x)  {
    for (uint8_t id: {M1, M2, M3}) {
      if (dxl.ping(id)) {
        Serial.println("");
        Serial.print("Motor ");
        Serial.print(id);
    ...
    // DO NOT delete this to end task or it will crash
    vTaskDelete( NULL );
  }
}
...
void setup() 
{
  ...
  xTaskCreate(pingMotor,        "Ping Motor",         256, NULL, tskIDLE_PRIORITY + 1, &Handle_pingMotorTask);
